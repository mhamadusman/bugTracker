import { createBug, updateBug } from '../types/types';
...
export class BugUtil {
  static async validateUpdateRequest(data: Bug) {
    ...
    if (!data.title || data.title.trim() === '') {
      throw new Exception('Title is required and cannot be empty', errorCodes.BAD_REQUEST);
    }

    if (!data.description || data.description.trim() === '') {
      throw new Exception('Description is required and cannot be empty', errorCodes.BAD_REQUEST);
    }
    ...
    if (!data.type || !Object.values(BugType).includes(data.type)) {
      throw new Exception(`Invalid Bug Type. Provided: ${data.type}`, errorCodes.BAD_REQUEST);
    }
  ...
  static async validateBugTitle(title: string, projectId: number , bugId? : number): Promise<void> {
  ...
  static async getBugs(projectId: number, userId: number, userType: string , page: number , limit: number): Promise<Bug[]> {
