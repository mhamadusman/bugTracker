...
export class projectManager {
  static async createProject(data: createProject, imgurl: string, managerId: number): Promise<project> {
    ...
    const newProject: project = await ProjectHandler.createProject(data, imgurl, managerId);
    return newProject;
  }
  static async getProjects(userId: number, userRole: string, page: number, limit: number): Promise<IProjects> {
    let result: IProjects;
    if (userRole === 'manager') {
      return result = await ProjectHandler.getManagerProjects(userId, page, limit);
    } else if (userRole === 'sqa') {
      return result = await ProjectHandler.getSQAprojects(userId, page, limit);
    } else if (userRole === 'developer') {
      return result = await ProjectHandler.getDeveloperProjects(userId, page, limit);
    }
  ...
  static async editProject(projectId: number, projectData: createProject, managerId: number, imgURL: string) {
    //validate project id
